...
## Introduction
The impact of human disturbance on Bornean stream frogs was assed by comparing the functional diversity, the functional trait variability and the functional trait composition between primary forests(PF), logged forests (LF) and oil palm plantations (OP). I started to measure the functional diversity of those habitats with three measures: functional richness, functional evenness and functional divergence. 

## A. Functional diversity:  Functional richness
The FD measure of functional richness described by Petchey and Gaston (2002,2006) and used by Riemann (2017) will be used in this study. Four steps are required for this calculation wich are 1) create a trait matrix, 2) convert the trait matrix into a distance matrix, 3) create a dendrogram based on the distances, 4) Measure the total branch length of the dendrogram (Petchey and Gaston, 2002). 

### 1) Trait matrix
Load usefull r package,  load the frog dataset from (Konopik, 2015) and create subsets removing NA for the snout-vent length (SVL) and the gape width (GW) traits
...
library(ggplot2)
library(tidyr)
Data<-readWorkbook("functional_frog_data.xlsx", sheet=1, colNames = TRUE, startRow = 10)
...
```
The difference is significant, which means that there is a sexual dimorphism between males and females. Therefore, the svl column will be split into males and females to create the trait matrix.
...
```
The difference is not significant, which means that there is no sexual dimorphism between males and females. Therefore, the gw column will be merged in one column to create the trait matrix.
```{r}
...
#### Gut content (GC)
The gut content of the frogs was available and I measured the class richness of the gut content and inserted it in the trait dataset since gut content is a direct information of resource use patterns (Petchey and Gaston, 2006).

```{r}
gut_data<-readWorkbook("functional_frog_data.xlsx", sheet=2, colNames = TRUE, startRow = 10)
...
#### Traits
```{r}
Matrix<-readWorkbook("functional_frog_data.xlsx", sheet=4, colNames = TRUE,rowNames = TRUE)
Matrix$BH <-as.factor(Matrix$BH)
Matrix$CP <-as.factor(Matrix$CP)
Matrix$RM <-as.factor(Matrix$RM)
Matrix$BC <-as.factor(Matrix$BC)
Matrix$RHL <-factor(Matrix$RHL, ordered=TRUE,levels=c("1","2","3","4"))
Matrix$HW <-factor(Matrix$HW, ordered=TRUE,levels=c("1","2","3","4"))
Matrix$FW <-factor(Matrix$FW, ordered=TRUE,levels=c("1","2","3","4"))
Matrix$TD <-factor(Matrix$TD, ordered=TRUE,levels=c("1","2","3","4"))
...
The function daisy helped to convert the trait matrix into a distance matrix based on Gower's distance that allows for mixed trait types (Swenson 2014). This function also standardised the trait values to enable equal comparisons. 
...
```{r}
distance_matrix<-daisy(Matrix,metric = "gower")
...
The Pearson's product-moment correlation between cophenetic (branch lenghts) and original distances was then calculated to verify that the tree was a correct representation of the reality. 
...
frogs_dendrogram<- set(frogs_dendrogram, "labels_cex", 0.5)
plot(frogs_dendrogram)
...
The functional richness (FD) is defined as the total branch length in a trait dendrogram connecting all species, but excluding the unnecessary root segments of the tree (Petchey and Gaston 2006). The community matrix was first created to know the abundance of forg species in each community.
```{r}
...
The function treedive has been adapted to this study to measure the functional richness (Petchey 2002, Riemann 2017). 
```{r}
div <- numeric(nrow(community))
for (i in 1:nrow(community)) {
  ...
  div[i] <- treeheight(cl_hclust)
  }
names(div) <- rownames(community)
div<-as.data.frame(div)
div

```
### Results functional richness
The influence of habitat modification on frogs functional richness was tested. The normality was tested to chose between parametric and non parametric tests. Since the normality was verified, I used an Anova to compare the habitats wich was significative. I finally did a tukey post hoc test to find wich habitats had a difference of FD. There is a significant reduction of functional diversity from PF to OP.


#### Influence of habitat on functional richness
```{r}
FRic<-readWorkbook("functional_frog_data.xlsx", sheet=5, colNames = TRUE) # The habitat type (PF, LF or OP) was added to the functional richness results
...
Functional diversity is generally highlty correlated to taxonomic richness (Petchey 2002). 
```{r}
...
Because functional richness was highly correlated with species richness, the latest was kept constant to allow comparison between communities to investigate variations of functional richnness beyond species richness (Viller et al.,  2008; Swenson, 2014; Riemann et al., 2017). The dendrogram tips were shuffled to randomize the functional traits while the functional richness did not change. The standardized effect size (SES), comparing the expected functional richness null model and the observed functional richness, was measured for each habitat. The function suffle the tips but change the order of the tips and needed to be ordered alphabetically.
...
The mean of the expected functional richness (FRic_exp) was compared to the observed functional richness per habitat type to assess deviation from the null model.
```{r,warning=FALSE, message=FALSE}
library(genefilter)
```
...
The normality of the distribution was verified for each habitat type. 
```{r}
# PF
...
The standardised effect size was compared accross the habitats to test if there is an influence of the habitat on the functional richness SES. Since the normality was respected, an anova was carried out.
```{r}
...
The functional evenness and the functional divergence of Bornean frogs were calculated with the indices developped by Villeger et al. (2008) and improved by Laliberte and Legendre (2010). 
```{r,warning=FALSE, message=FALSE}
library(FD)
library(picante) 
```
...
#### Null models functional evenness and functional divergence
...
To understand investigate wheter some functional traits vary across the different habitats, the community weighted mean of each trait was compared between all the habitat types. It is defined as the mean value of each trait weighted by the relative abundance of each species (Lavorel et al., 2008). Traits with parametric distributions were evaluated with a one-way ANOVA followed by a Tukey post-hoc test. Traits with non-parametric distributions were evaluated with a ruskal Wallis analysis of variance test followed by a Nemenyi post-hoc test.
```{r}
Matrix2<-readWorkbook("functional_frog_data.xlsx", sheet=4, colNames = TRUE,rowNames = TRUE)
community<-as.matrix(community)
CWM2<-functcomp(Matrix2,community)
CWM_table<-readWorkbook("functional_frog_data.xlsx", sheet=6, colNames = TRUE)
CWM_table$Habitat <- factor(CWM_table$Habitat , levels=c("PF", "LF", "OP"))
```

```{r,eval=FALSE}
#cp
shapiro.test(CWM_table$CP)
boxplot(CWM_table$CP~CWM_table$Habitat,xlab="Habitat",ylab="CWM_CP")
Mod_cwm_cp<-kruskal.test(CWM_table$CP~CWM_table$Habitat)
Mod_cwm_cp

#BH
shapiro.test(CWM_table$BH)
boxplot(CWM_table$BH~CWM_table$Habitat,xlab="Habitat",ylab="CWM_BH")
Mod_cwm_BH<-aov(CWM_table$BH~CWM_table$Habitat)
summary(Mod_cwm_BH)

#Mean male SVL
shapiro.test(CWM_table$Mean.male.SVL)
boxplot(CWM_table$Mean.male.SVL~CWM_table$Habitat,xlab="Habitat",ylab="CWM_Mean.male.SVL")
Mod_cwm_Mean.male.SVL<-kruskal.test(CWM_table$Mean.male.SVL~CWM_table$Habitat)
Mod_cwm_Mean.male.SVL

#Mean female SVL
shapiro.test(CWM_table$Mean.female.SVL)
boxplot(CWM_table$Mean.female.SVL~CWM_table$Habitat,xlab="Habitat",ylab="CWM_Mean.female.SVL")
Mod_cwm_Mean.female.SVL<-kruskal.test(CWM_table$Mean.female.SVL~CWM_table$Habitat)
Mod_cwm_Mean.female.SVL

#GW
shapiro.test(CWM_table$GW)
boxplot(CWM_table$GW~CWM_table$Habitat,xlab="Habitat",ylab="CWM_GW")
Mod_cwm_GW<-kruskal.test(CWM_table$GW~CWM_table$Habitat)
Mod_cwm_GW

#RHL
shapiro.test(CWM_table$RHL)
boxplot(CWM_table$RHL~CWM_table$Habitat,xlab="Habitat",ylab="CWM_RHL")
Mod_cwm_RHL<-aov(CWM_table$RHL~CWM_table$Habitat)
summary(Mod_cwm_RHL)

#HW
shapiro.test(CWM_table$HW)
boxplot(CWM_table$HW~CWM_table$Habitat,xlab="Habitat",ylab="CWM_HW")
Mod_cwm_HW<-aov(CWM_table$HW~CWM_table$Habitat)
summary(Mod_cwm_HW)

#FW
shapiro.test(CWM_table$FW)
boxplot(CWM_table$FW~CWM_table$Habitat,xlab="Habitat",ylab="CWM_FW")
Mod_cwm_FW<-aov(CWM_table$FW~CWM_table$Habitat)
summary(Mod_cwm_FW)

#TD
shapiro.test(CWM_table$TD)
boxplot(CWM_table$TD~CWM_table$Habitat,xlab="Habitat",ylab="CWM_TD")
Mod_cwm_TD<-aov(CWM_table$TD~CWM_table$Habitat)
summary(Mod_cwm_TD)

#RM
shapiro.test(CWM_table$RM)
boxplot(CWM_table$RM~CWM_table$Habitat,xlab="Habitat",ylab="CWM_RM")
Mod_cwm_RM<-kruskal.test(CWM_table$RM~CWM_table$Habitat)
Mod_cwm_RM

#BC
shapiro.test(CWM_table$BC)
boxplot(CWM_table$BC~CWM_table$Habitat,xlab="Habitat",ylab="Colour (community weighted mean)", ylim=c(1,4))
Mod_cwm_BC<-aov(CWM_table$BC~CWM_table$Habitat)
summary(Mod_cwm_BC)
Tukey_BC<-TukeyHSD(x=Mod_cwm_BC, 'CWM_table$Habitat', conf.level=0.95,)
Tukey_BC

#GC
shapiro.test(CWM_table$GC)
boxplot(CWM_table$GC~CWM_table$Habitat,xlab="Habitat",ylab="CWM_GC")
Mod_cwm_GC<-aov(CWM_table$GC~CWM_table$Habitat)
summary(Mod_cwm_GC)
Tukey_GC<-TukeyHSD(x=Mod_cwm_GC, 'CWM_table$Habitat', conf.level=0.95)
Tukey_GC
```
...
Functional composition was first assessed with the functional integrity index developped by De Coster et al.(2015). The community weighted mean dataset was used to calculate the pairwise distance between sites based on the gower distance. The resulting distance matrix was reduced by a classical multidimensional scaling ordination. Functional integrity is measured by substracting the mean values of primary forests sites from each logged forest site and each oil palm plantation sites.

```{r}
dist_cwm<-as.matrix(vegdist(CWM2,method="gower", binary=FALSE))
...
Functional compostion of Bornean frogs was then measured with the tree-based beta-diversity index which can be used to compute  functional beta-diversity, a measure of the trait composition dissimilarity between a pair of communities (Swenson, 2014). First, the unique (non-shared) fraction of the functional tree was measured for each community pair using the unifrac metric. 
...
### NMDS 
A non-metric multidimensional scaling ordination method was used to visualise the dissimilarity of trait composition between communities measured by functional beta-diversity.  
```{r}
...
points(ordiplot,what = "sites",bg=CWM_table$Habitat,pch=21)
